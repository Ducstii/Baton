...
func runHarness(repoDir, prompt, providerID, modelID string) {
	if repoDir == "" || prompt == "" {
		fmt.Fprintf(os.Stderr, "usage: baton -harness -repo <path> -prompt <text> [-provider <id>] [-model <id>]\n")
		os.Exit(1)
	}
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "OpenCode not reachable: %v\n", err)
		os.Exit(1)
	}
